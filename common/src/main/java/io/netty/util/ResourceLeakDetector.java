...
public class ResourceLeakDetector<T> {
    ...
    static final int SAMPLING_INTERVAL;// 默认=simple 采样间隔

    /**
     * Represents the level of resource leak detection.
     */
    public enum Level {  // 内存检测的 强度 越下面强度越高
        /**
        ...
        static Level parseLevel(String levelStr) {
            ...
            for (Level l : values()) {
                if (trimmedLevelStr.equalsIgnoreCase(l.name()) || trimmedLevelStr.equals(String.valueOf(l.ordinal()))) {// index enum
                    return l;
                }
            }
    ...
    static { // 默认 的 检查等级是 simple
        ...
        levelStr = SystemPropertyUtil.get(PROP_LEVEL, levelStr); // 对老的netty版本 做一个向下  xian rong
    ...
    private DefaultResourceLeak track0(T obj) {
        ...
        //  检测Level 是否 小于 PARANOID
        if (level.ordinal() < Level.PARANOID.ordinal()) {
            //samplingInterval 间隔时间 默认是128  当=0 的时候 才会随机采样 要想 必采样 设置 jvm参数=1;
            if ((PlatformDependent.threadLocalRandom().nextInt(samplingInterval)) == 0) {
                reportLeak();
                return new DefaultResourceLeak(obj, refQueue, allLeaks); // 传入检测的 对象 ，弱引用队列，所有的 检测器，
            }
            return null;
        }
        reportLeak();
    ...
    private void reportLeak() {
        ...
        for (;;) {
            @SuppressWarnings("unchecked")
            DefaultResourceLeak ref = (DefaultResourceLeak) refQueue.poll(); // 如果是 抽取采样那么 在 第二次采样才会打印之前采样的结果。
            if (ref == null) {
                break;
            }

            if (!ref.dispose()) { //如果在 gc之前 进行了 释放操作 那么 这儿 就返回 false 否则返回true 代表发生了 泄漏
                continue;
            }
    ...
    private static final class DefaultResourceLeak<T>
        ...
        DefaultResourceLeak(Object referent, ReferenceQueue<Object> refQueue, Set<DefaultResourceLeak<?>> allLeaks) {
            super(referent, refQueue); // 对检测的 bytebuff 弱引用
        ...
        private void record0(Object hint) {
            ...
            if (TARGET_RECORDS > 0) {
                ...
                do {
                    ...
                    // 这是一个方向的 单向链表，最新的一个Node 会成headNode，指向老的Node 【pos属性等于 当前节点 在整个链表中的第几个】
                    // 从0 开始 最后一个next  就是bottom pos= -1 由此来判断是否 next到底了
        ...
        public boolean close(T trackedObject) {
            ...
            try {
                ...
                // 防止当前对象 在close之前 追踪对象本身 也就是this  被回收了 这个跟fialzz那个方法一样 不保证执行完。
                // 也就是说 如果 手动reasle之后 确保 能将 this 在 Leaks中 remove掉 防止出现误判。
            }
        }
        ...
        public String toString() {
            Record oldHead = headUpdater.getAndSet(this, null); // 置空。
            if (oldHead == null) {
                // Already closed
                return EMPTY_STRING;
            }
