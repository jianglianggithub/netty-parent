...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    protected SingleThreadEventExecutor(EventExecutorGroup parent, Executor executor,
        ...
        // 这个地方 会重写 Excecutor 得 exceutor方法  目的是为了 在 执行对应得  runnble 任务得时候 会把当前任务 执行对应得 NioEventLoop
        //暴露出来 能够 有机会用到。

        /**
         *
         *  其实在EventLoop 中 是看不到线程的 而 当提交第一个任务的时候 new Thread.start 执行的就是 EventLoop 中 run方法 轮训 take task
         */
        this.executor = ThreadExecutorMap.apply(executor, this);
    ...
    private boolean fetchFromScheduledTaskQueue() {
        ...
        for (;;) {
            Runnable scheduledTask = pollScheduledTask(nanoTime); //取出定时任务 并且 执行时间是<=当前时间
            if (scheduledTask == null) {
                return true;
            }
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        // 将所有 在定时任务执行时间 <= 当前时间的 任务 加入到  taskQueue中
        fetchFromScheduledTaskQueue();
        ...
        for (;;) {
            safeExecute(task); // run 同步执行

            runTasks ++;

            // Check timeout every 64 tasks because nanoTime() is relatively expensive.
            // XXX: Hard-coded value - will make it configurable if it is really a problem.
            if ((runTasks & 0x3F) == 0) { //   当 runTasks = 1000000 时  &  63  那么就等于0 也就是64次检查一次超时  所以这个超时并不是绝对的
                lastExecutionTime = ScheduledFutureTask.nanoTime();
    ...
    private void doStartThread() {
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                //让线程工厂 executor 去 start一个线程去执行 runnble 然后 在执行的时候 把对应的线程绑定到 NioServerSocketChannel socketChannel
                thread = Thread.currentThread();
                if (interrupted) { // 如果收到中断请求 那么
                    thread.interrupt();// 中断
                }
                ...
                try {
                    ...
                    /**
                     *  这个地方 似乎是 为了 实现  运行期间 得中断方法得具体实现 ExecutorService 接口中
                     */
                    try {
