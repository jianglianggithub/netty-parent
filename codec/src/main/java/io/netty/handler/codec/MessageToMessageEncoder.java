    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        try {
            if (acceptOutboundMessage(msg)) {
                ...
                I cast = (I) msg;
                try {
                    encode(ctx, cast, out);  // 调用 编码器的  encode方法
                } finally {
            ...
            if (out != null) { //
                ...
                if (sizeMinusOne == 0) {
                    ctx.write(out.getUnsafe(0), promise); // 如果只有一条消息 那么 调用  outBound 的下一个witer
                } else if (sizeMinusOne > 0) {  //有多个messge 那么 就调用多次  unsafe 的 witer
