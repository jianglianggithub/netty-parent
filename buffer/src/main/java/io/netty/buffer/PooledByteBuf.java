...
abstract class PooledByteBuf<T> extends AbstractReferenceCountedByteBuf {
    ...
    // 把一个bytebuff 当做个 bytebuff用 每个  bytebuff有自己的 posion 指针 和limit
    ...
    final ByteBuffer _internalNioBuffer(int index, int length, boolean duplicate) {
        ...
        buffer.limit(index + length).position(index);// 设置 当前 使用的  Bytebuff 中的 posion 和limit  每个分区 有自己的 posion 和limit
        return buffer;
    }
