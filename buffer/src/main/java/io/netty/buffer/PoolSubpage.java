...
final class PoolSubpage<T> implements PoolSubpageMetric {
    ...
    int elemSize;// 该 subpage分区 中每个小分区的 length
    private int maxNumElems; // 最大可用的小分区个数

    // bitMap 默认是 8个long 一个long 表示64个  subpage 最大分区= 8192/16 =512个 所以 默认是 64*8
    // 这里的 length 代表有几个long 来表示 subpage  中 每个小分区 是否被使用
    private int bitmapLength;

    private int nextAvail;
    private int numAvail; // 该subPage 剩下可用的 个数
    ...
    long allocate() {
        ...
        // 通过算法得到   当前使用subPage 中的小段快缓冲区 处于该subpage 第几个元素
        final int bitmapIdx = getNextAvail();
        int q = bitmapIdx >>> 6;
        int r = bitmapIdx & 63;
        assert (bitmap[q] >>> r & 1) == 0;
        bitmap[q] |= 1L << r;// 将 64位的long 对应的 subpage中的小分区 =1 代表已使用
    ...
    boolean free(PoolSubpage<T> head, int bitmapIdx) {
        ...
        // 如果之前 该Node 可使用的 小分区为0 就会从 对应的分区链表中删除 那么 现在 恢复一个Node 然后将 当前node 添加到 分区链表中
        if (numAvail ++ == 0) {
            addToPool(head);
            return true;
        }
