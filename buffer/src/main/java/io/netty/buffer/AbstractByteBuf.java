 ...
 *              Unpooled
 *                  Pooled
 *                                  ---- 池化 与 非池化区别 在于。 是否从 已经申请好的内存中 引用一块连续的内存 避免了重复的申请内存和释放内存 带来的消耗
 *              Direct
 *              Heap
 *                                  -----  建立在 jvm堆上 还是使用用户态的直接内存
 *
 *              Unsafe
 *              非Unsafe
 *                                  -----  如果是unsafe 那么创建缓冲区 和获取缓冲区 对应的byte 都是基于 Unsafe 通过内存offset 来获取的
  *                                               不使用unsafe 那么就只能通过 jdk原生的 api 来操作缓冲区
 */
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    // 内存泄漏追踪器工厂 ， 里面实现了扩展 可以在 jvm 参数中指定自己的 内存泄漏 追中器。如果没有 默认是DefualtResourceLeakDetector
