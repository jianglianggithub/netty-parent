...
public class ServerBootstrap extends AbstractBootstrap<ServerBootstrap, ServerChannel> {
    ...
    void init(Channel channel) {
        ...
        p.addLast(new ChannelInitializer<Channel>() {//这个 处理器负责 在将 serverSocket注册到选择器上之后 做一个 回调
            @Override                                                       //  注册到选择器上之后 会反过来调用注册之前添加到 NIoServerSocketChannel中PipeLine中的ctx中的handle
            public void initChannel(final Channel ch) {//NioServerSocketChannel
                final ChannelPipeline pipeline = ch.pipeline();
                // config 是专门向外暴露 用于 获取的 对象
                ...
                ch.eventLoop().execute(new Runnable() {
                    @Override
                    public void run() {
                        System.out.println("   添加 Accpet Handle");
