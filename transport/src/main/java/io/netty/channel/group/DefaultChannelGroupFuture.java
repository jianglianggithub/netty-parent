...
final class DefaultChannelGroupFuture extends DefaultPromise<Void> implements ChannelGroupFuture {
    ...
    // groupFutrue 和 单个 futrue的区别，就是 在于 后续lisitener 的通知是不同的
    // 而且这个 lisitener 会添加到 所有 组中的 channel之中 ，全部成功或者 失败之后 会 异步通知
    private final ChannelFutureListener childListener = new ChannelFutureListener() {
        @Override
        public void operationComplete(ChannelFuture future) throws Exception {
            boolean success = future.isSuccess();
            boolean callSetDone;// 默认是fasle
            ...
            if (callSetDone) {
                if (failureCount > 0) {// 如果出现异常
                    ...
                    setFailure0(new ChannelGroupException(failed));// 将失败的 channel 和异常 传入 待处理
                } else {
                    setSuccess0();
                }
            }
