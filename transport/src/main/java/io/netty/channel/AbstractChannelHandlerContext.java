...
/**
 * 这个class 的方法 是所有事件触发  后 递归调用 管道中双向链表 保存的所有ctx 中handle 的对应方法 的 中转站
 */
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    private void invokeChannelRegistered() {
        if (invokeHandler()) {
            try { // 如果是头 或者尾部  this= Head  或者 tial
    ...
    @Override //这个方法 默认从 Tail -----> head 然后真正bind  看server注释
    public ChannelFuture bind(final SocketAddress localAddress, final ChannelPromise promise) {
        ...
        // 这个是一个  类似 AOP 和 注册之后 把所有管道中的 处理器 全部执行的  那种思想。
        // 也就是说  bind方法 也是一个 轮询 管道的链表中的所有结点的bind方法  然后 走ctx中对应处理器的bind方法
        // 如果想后续的   handle 也执行 那么就ctx .dobind 那么 会有一个轮询
    ...
    /**
     * 重Head 开始 ChannelActive 然后 走到Tail 直接 是一个empty方法。
     * @return
     */
    @Override
    public ChannelHandlerContext read() {
        // 查找outBound  类型的 handle   之所以从头走到尾部的 ChannelActive方法 后又从尾 走到头原因是 扩展
    ...
    // 使用 ctx.channel.writeAndFush 会从 Tail节点开始 从回轮询 而  ctx.writeAndFush 直接从当前节点往回轮询
