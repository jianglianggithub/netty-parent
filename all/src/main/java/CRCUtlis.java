...
//        8) 最后得到的 CRC寄存器内容即为 CRC码。
    ...
    public static void main(String[] args) {
        ...
        // 70 E8 FF FF FF FF 00 32 CA 00 00 00 01 01 00 01 08 03 00
            String strings = "70 E8 FF FF FF FF 00 32 CA 00 00 00 01 01 00 01 08 03 00";
        System.out.println("70 E8 FF FF FF FF 00 32 CA 00 00 00 01 01 00 01 08 03 00".replace(" ",""));
        ByteBuf buffer = Unpooled.buffer(1204);
//        System.out.println(Integer.valueOf("0x1F".replace("0x",""),16));
        String[] s = strings.split(" ");
        int a = 0;
        for (String s1 : s) {
            int integer = Integer.valueOf(s1, 16);
            allocate.put((byte) integer);
            a++;
        }
        byte[] array = allocate.array();
        byte[] ary19 = new byte[19];
        System.arraycopy(array,0,ary19,0,19);
        String crc = getCRC(ary19);
        String crc1 = getCRC3(ary19);
        String crc2 = getCRC2(ary19);
        int i = CRC16_MODBUS(ary19);
        System.out.println(Integer.toHexString(i)+" 老哥发的");
        System.out.println(crc +" getCRC ");
        System.out.println(crc1 +" getCRC3 ");
        System.out.println(crc2 +" getCRC2 ");

    }
      public static String getCRC4(byte[] bytes) {
                 int CRC = 0xFFFF;
                 int POLYNOMIAL = 0xA001;
                 int i, j;
                 for (i = 0; i < bytes.length; i++) {
